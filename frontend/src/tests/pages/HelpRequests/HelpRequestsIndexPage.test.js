...
describe("HelpRequestIndexPage tests", () => {
  ...
  test("renders three helprequests correctly for regular user", async () => {
    ...
    const requesterEmail = screen.getByText("joe@gmail.com");
    expect(requesterEmail).toBeInTheDocument();

    const teamId = screen.getByText("A");
    expect(teamId).toBeInTheDocument();

    const tableOrBreakoutRoom = screen.getByText("table");
    expect(tableOrBreakoutRoom).toBeInTheDocument();

    const requestTime = screen.getByText("2025-12-10T06:34:12");
    expect(requestTime).toBeInTheDocument();

    const explanation = screen.getByText("need help!");
    expect(explanation).toBeInTheDocument();

    // const solved = screen.getByText("joe@gmail.com");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(
        "HelpRequest with id 3 was deleted",
      );
    });
